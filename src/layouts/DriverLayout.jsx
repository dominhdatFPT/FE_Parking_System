...
export default function DriverLayout() {
  ...
  return (
    <div className="relative flex h-screen overflow-hidden bg-[radial-gradient(circle_at_top_right,rgba(232,121,249,0.18),transparent_50%),radial-gradient(circle_at_bottom_left,rgba(14,165,233,0.24),transparent_55%),linear-gradient(135deg,#f8fafc_0%,#dbeafe_45%,#f3e8ff_100%)] text-[#0F172A]">
      <div className="pointer-events-none absolute right-0 top-0 z-0 h-48 w-48 opacity-[0.32]" aria-hidden="true">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="driver-layout-dots" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
              <circle cx="3" cy="3" r="2" fill="#818cf8" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#driver-layout-dots)" />
        </svg>
      </div>
      <div className="pointer-events-none absolute bottom-0 left-0 z-0 h-64 w-64 opacity-[0.34]" aria-hidden="true">
      ...
      <div className="relative z-10 flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="min-h-full w-full space-y-6 p-4 lg:p-7">
            <Outlet />
          </div>
        </main>
      </div>
